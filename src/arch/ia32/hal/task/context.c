...
static void switch_page_dir(ulong page_dir_pfn)
{
...
static void switch_to_user(struct context *context)
{
    __asm__ __volatile__
    (
...
static void switch_to_kernel(struct context *context)
{
    kprintf("To switch, target ESP: %p\n", context->esp);
    
    struct context *dest = (struct context *)(context->esp - sizeof(struct context));
    memcpy(context, dest, sizeof(struct context));
