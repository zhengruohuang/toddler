...
void init_kmem_zone()
{
     ...
     */
    kprintf("\tFirmware reported usable zones\n");
     ...
     */
    // Zone 0: Low 1MB for exception handler, unusable, mapped, inuse
    mem_zones[0].start = 0;
    ...
    // Go through firmware reported zones
    ...
    for (i = 0; i < get_bootparam()->mem_zone_count; i++) {
        ...
        if (cur_zone->type) {
            ...
            mem_zones[mem_zone_count].usable = 0;
            mem_zones[mem_zone_count].mapped = 1;
            mem_zones[mem_zone_count].tag = 0;
            mem_zones[mem_zone_count].inuse = 1;
            mem_zones[mem_zone_count].kernel = 1;
            mem_zones[mem_zone_count].swappable = 0;
        }
