...
#define DEVFS_BLOCK_SIZE    128


struct devfs_block {
    struct devfs_block *prev;
    struct devfs_block *next;
    
    u8 data[DEVFS_BLOCK_SIZE];
};
...
struct devfs_data {
    ...
    struct devfs_block *head;
    struct devfs_block *tail;
};
 ...
 * Data
 */
static int seek_data_block(struct devfs_data *data, unsigned long pos, unsigned long *real_pos, struct devfs_block **cur_block, unsigned long *cur_offset)
{
    struct devfs_block *block = data->head;
    ...
    offset = pos;
    while (offset >= DEVFS_BLOCK_SIZE) {
        offset -= DEVFS_BLOCK_SIZE;
        assert(block);
        block = block->next;
    }
...
static unsigned long read_data_block(struct devfs_data *data, unsigned long pos, u8 *buf, unsigned long count)
{
    int index = 0;
...
static unsigned long write_data_block(struct devfs_data *data, unsigned long pos, u8 *buf, unsigned long count)
{
    int index = 0;
    unsigned long cur_pos = pos;
...
//     kprintf("write, buf: %s, count: %p\n", buf, count);
    ...
    while ((unsigned long)index < count) {
        ...
        if (!block) {
            block = (struct devfs_block *)salloc(block_salloc_id);
            offset = 0;
...
static int truncate_data_block(struct devfs_data *data, unsigned long pos)
{
    unsigned long cur_pos = pos;
    struct devfs_block *block;
    unsigned long offset = 0;
    ...
    block = block->next;
    while (block) {
        struct devfs_block *next = block->next;
        sfree(block);
        data->block_count--;
        block = next;
    }
...
static void free_data_block(struct devfs_data *data)
{
    // Free all blocks
    struct devfs_block *block = data->head;
    while (block) {
 ...
 * Helpers
 */
static struct devfs_node *get_node_by_id(unsigned long super_id, unsigned long node_id)
{
    return (struct devfs_node *)node_id;
}

static struct devfs_node *create_node(const char *name, struct devfs_node *parent)
{
    struct devfs_node *node = (struct devfs_node *)salloc(node_salloc_id);
...
static struct devfs_open *get_open_by_id(unsigned long open_id)
{
    return (struct devfs_open *)open_id;
}

static struct devfs_open *create_open(unsigned long super_id, unsigned long node_id)
{
    struct devfs_node *node = get_node_by_id(super_id, node_id);
    if (!node) {
        return NULL;
    }
    
    struct devfs_open *open = (struct devfs_open *)salloc(open_salloc_id);
...
{
    struct devfs_node *node = NULL, *next = NULL;
...
//     kprintf("Lookup received, super_id: %p, node_id: %lu, name: %s\n", super_id, node_id, name);
    ...
    if (node_id) {
        ...
        next = (struct devfs_node *)hash_obtain(node->sub.entries, (void *)name);
        if (!next) {
            hash_release(node->sub.entries, (void *)name, next);
            return 0;
        }
    } else {
        node = (struct devfs_node *)hash_obtain(devfs_table, (void *)super_id);
        hash_release(devfs_table, (void *)super_id, node);
        next = node;
        assert(next);
    }
...
static int open(unsigned long super_id, unsigned long node_id, unsigned long proc_id, unsigned long *open_id)
{
    ...
    struct devfs_open *open = create_open(super_id, node_id);
    if (!open) {
        return EBADF;
    }
...
{
    struct devfs_open *open = get_open_by_id(open_id);
    if (!open) {
        return EBADF;
    }
...
static int write(unsigned long super_id, unsigned long open_id, void *buf, unsigned long count, unsigned long *actual)
{
    unsigned long result = 0;
...
static int truncate(unsigned long super_id, unsigned long open_id)
{
    int result = 0;
...
static int create(unsigned long super_id, unsigned long open_id, char *name, enum urs_create_type type, unsigned int flags, char *target, unsigned long target_open_id)
{
    struct devfs_node *sub = NULL;
    ...
    case ucreate_hard_link: {
        struct devfs_open *target_open = get_open_by_id(target_open_id);
        if (!target_open) {
            return EBADF;
        }
        sub = open->node;
...
static int remove(unsigned long super_id, unsigned long open_id, int erase)
{
...
static int rename(unsigned long super_id, unsigned long open_id, char *name)
{
    struct devfs_node *parent = NULL;
 ...
 * Message handler
 */
asmlinkage void devfs_msg_handler(msg_t *msg)
{
    int result = EOK;
    ...
    case uop_lookup: {
        ...
        unsigned long proc_id = msg->params[3].value;
        char *name = (char *)((unsigned long)msg + msg->params[4].offset);
        int is_link = 0;
        unsigned long next_id = 0;
        
        result = lookup(super_id, node_id, proc_id, name, &is_link, &next_id, NULL, 0, NULL);
...
//         kprintf("name: %s, node: %p, is link: %d, next: %p\n", name, node_id, is_link, next_id);
    ...
    case uop_open: {
        ...
        result = open(super_id, node_id, proc_id, &open_id);
    ...
    case uop_read: {
        ...
        result = read(super_id, open_id, buf, count, &actual);
    ...
    case uop_write: {
        ...
        result = write(super_id, open_id, buf, count, &actual);
    ...
    case uop_truncate: {
        unsigned long open_id = msg->params[2].value;
        result = truncate(super_id, open_id);
        break;
    }
    ...
    case uop_seek_data: {
        ...
        result = seek_data(super_id, open_id, offset, from, &newpos);
    ...
    case uop_list: {
        ...
        result = list(super_id, open_id, buf, count, &actual);
    ...
    case uop_seek_list: {
        ...
        result = seek_list(super_id, open_id, offset, from, &newpos);
    ...
    case uop_create: {
        ...
        result = create(super_id, open_id, name, type, flags, target, target_open_id);
        break;
    }
    ...
    case uop_remove: {
        ...
        result = remove(super_id, open_id, erase);
        break;
    }
    ...
    case uop_rename: {
        unsigned long open_id = msg->params[2].value;
        char *name = (void *)((unsigned long)msg + msg->params[3].offset);
        result = rename(super_id, open_id, name);
        break;
    }
    ...
    case uop_stat: {
        ...
        result = stat(super_id, open_id, &s);
...
#define REG_OP(op, handler) do {                                        \
    ...
    syscall_reg_msg_handler(func_num, devfs_msg_handler);               \
} while (0)

static int register_devfs(char *path)
{
    struct urs_reg_ops ops;
    struct devfs_node *root = NULL;
    unsigned long super_id = 0;
    
    // Prepare operations
    REG_OP(uop_lookup, lookup);
    REG_OP(uop_open, open);
    REG_OP(uop_release, close);
    ...
    // Register the FS
    super_id = kapi_urs_reg_super(path, "devfs", 0, &ops);
    if (!super_id) {
        return -2;
    }
    ...
    hash_insert(devfs_table, (void *)super_id, root);
...
void init_devfs()
{
    devfs_table = hash_new(0, NULL, NULL);
    open_salloc_id = salloc_create(sizeof(struct devfs_open), 0, NULL, NULL);
    node_salloc_id = salloc_create(sizeof(struct devfs_node), 0, NULL, NULL);
    block_salloc_id = salloc_create(sizeof(struct devfs_block), 0, NULL, NULL);
...
//     char buf[128];
//     
//     kprintf("To register DevFS @ /test/devfs\n");
//     register_devfs("/test/devfs");
//     kprintf("Registered!");
//     
//     kprintf("To open URS node");
//     unsigned long f = urs_open_node("/test/devfs", 0, 0);
//     if (!f) {
...
//     unsigned long nn = urs_open_node("/test/devfs/obj1", 0, 0);
//     if (!nn) {
//         kprintf("Unable to open node!\n");
//         return;
//     }
